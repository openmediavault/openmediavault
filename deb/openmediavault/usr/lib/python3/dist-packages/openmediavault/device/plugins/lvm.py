...
class StorageDevicePlugin(openmediavault.device.plugins.dm.StorageDevicePlugin):
    def match(self, device_file):
        # Check if the given device is of type device mapper.
        if not super().match(device_file):
            return False
        sd = self.from_device_file(device_file)
        return sd.exists
...
class StorageDevice(openmediavault.device.plugins.dm.StorageDevice):
    """
