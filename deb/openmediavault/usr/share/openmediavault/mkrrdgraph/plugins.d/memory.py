...
class Plugin(openmediavault.mkrrdgraph.IPlugin):
	def create_graph(self, config):
		...
		args.append('GPRINT:umin:MIN:"%5.1lf%sB Min"'.format(**config))
		args.append('GPRINT:uavg:AVERAGE:"%5.1lf%sB Avg"'.format(**config))
		...
		args.append('GPRINT:bmin:MIN:"%5.1lf%sB Min"'.format(**config))
		args.append('GPRINT:bavg:AVERAGE:"%5.1lf%sB Avg"'.format(**config))
		args.append('GPRINT:bmax:MAX:"%5.1lf%sB Max"'.format(**config))
		args.append('GPRINT:bavg:LAST:"%5.1lf%sB Last\l"'.format(**config))
		args.append('AREA:cavg{color_memory_cached}:"Page cache  ":STACK'.format(**config))
		args.append('GPRINT:cmin:MIN:"%5.1lf%sB Min"'.format(**config))
		args.append('GPRINT:cavg:AVERAGE:"%5.1lf%sB Avg"'.format(**config))
		args.append('GPRINT:cmax:MAX:"%5.1lf%sB Max"'.format(**config))
		args.append('GPRINT:cavg:LAST:"%5.1lf%sB Last\l"'.format(**config))
		args.append('AREA:favg{color_memory_free}:"Free        ":STACK'.format(**config))
		args.append('GPRINT:fmin:MIN:"%5.1lf%sB Min"'.format(**config))
		args.append('GPRINT:favg:AVERAGE:"%5.1lf%sB Avg"'.format(**config))
