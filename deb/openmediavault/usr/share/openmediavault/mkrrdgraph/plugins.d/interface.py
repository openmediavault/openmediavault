...
class Plugin(openmediavault.mkrrdgraph.IPlugin):
	def create_graph(self, config):
		...
		for interface in interfaces:
			...
			args.append('GPRINT:oavg:AVERAGE:"%5.1lf%s Avg"'.format(**config))
			args.append('GPRINT:omax:MAX:"%5.1lf%s Max"'.format(**config))
			args.append('GPRINT:oavg:LAST:"%5.1lf%s Last"'.format(**config))
			args.append('GPRINT:otot:"%5.1lf%s Total\l"'.format(**config))
			args.append('LINE1:iavg{color_interface_incoming}:"Incoming"'.format(**config))
			args.append('GPRINT:iavg:AVERAGE:"%5.1lf%s Avg"'.format(**config))
			args.append('GPRINT:imax:MAX:"%5.1lf%s Max"'.format(**config))
			args.append('GPRINT:iavg:LAST:"%5.1lf%s Last"'.format(**config))
			args.append('GPRINT:itot:"%5.1lf%s Total\l"'.format(**config))
			args.append('COMMENT:"  Total   "')
			args.append('GPRINT:tavg:AVERAGE:"%5.1lf%s Avg"'.format(**config))
